...
class MyhomeFlowHandler(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_custom(self, user_input=None, errors={}):  # pylint: disable=dangerous-default-value
        ...
        address_suggestion = user_input["address"] if user_input is not None and user_input["address"] is not None else "192.168.2.17"
        port_suggestion = user_input["port"] if user_input is not None and user_input["port"] is not None else 20000
        serial_number_suggestion = user_input["serialNumber"] if user_input is not None and user_input["serialNumber"] is not None else "00:03:50:01:0C:37"
        model_name_suggestion = user_input["modelName"] if user_input is not None and user_input["modelName"] is not None else "F452"
